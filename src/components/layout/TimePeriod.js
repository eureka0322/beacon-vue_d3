...
export default {
  ...
  mounted() {
    ...
    nch.eventDispatcher.$on('timePeriodDataLoaded', function() {
      ...
      if(cachedTimePeriod) {
        ...
        component.setInitWeeklyRangeData()
  ...
  methods: {
    ...
    updateTimePeriodData: function() {
      ...
      this.weeklyRangeOptions = this.timePeriodService.getWeeklyRange()
    },
    ...
    setInitWeeklyRangeData: function() {
      let temp = []
      let that = this
      that.model.selectedItem.weekly_range.counter = 0
      this.weeklyRangeOptions.map(function(v) {
        v.values.map(function(k, i) {
          that.model.selectedItem.weekly_range.counter ++
          k.values.map(function(obj) {
            temp.push(obj)
          })
        })
      ...
      temp.length > 2
        ? (this.model.selectedItem.weekly_range.data = temp.slice(0, 2))
        : (this.model.selectedItem.weekly_range.data = temp)
    },
